# -----------------------------------------------------------------------------
# From model.py of nanoGPT: https://github.com/karpathy/nanoGPT
# Changes are marked by @psando, and described below:
# - use in_vocab_size and out_vocab_size, because our models reads both but 
#   only predicts audio
# - removed classmethod from_pretrained
# -----------------------------------------------------------------------------
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            wte = nn.Embedding(config.in_vocab_size, config.n_embd),
            wpe = nn.Embedding(config.block_size, config.n_embd),
            drop = nn.Dropout(config.dropout),
            h = nn.ModuleList([Block(config) for _ in range(config.n_layer)]),
            ln_f = LayerNorm(config.n_embd, bias=config.bias),
        ))
        self.lm_head = nn.Linear(config.n_embd, config.out_vocab_size, bias=False) # TODO: @psando: use num audio tokens here!
        # with weight tying when using torch.compile() some warnings get generated:
        # "UserWarning: functional_call was passed multiple values for tied weights.
        # This behavior is deprecated and will be an error in future versions"
        # not 100% sure what this is, so far seems to be harmless. TODO investigate
        # @psando: no weight tying because can't partially tie transformer.wte
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            ...
            loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=-100) # @psando: ignore_index=-100
        else:
    ...
    # @psando: removed classmethod from_pretrained(cls, model_type, override_args=None)
